...
metadata:
  yilin:
    tags: [system, admin, mcp, tool, external]
    related_skills: [model-provider-setup, session-management]
    prerequisites: [tool-constraint]
    conflicts_with: []
---

# MCP Setup Guide

> **路径说明**：Server 数据文件位于 `apps/server/data/`。Agent 需 workspace 指向项目根或有路径授权才能访问。

## MCP 配置存储位置

每个 MCP 服务器一个目录：`apps/server/data/mcpservers/<server_name>/config.json`。
...
1. 确定服务器名称（唯一，用作目录名）
2. 用 `write` 创建 `apps/server/data/mcpservers/<name>/config.json`
...
- 可通过 `edit` 直接修改 `apps/server/data/mcpservers/<name>/config.json` 修复后重开 session
