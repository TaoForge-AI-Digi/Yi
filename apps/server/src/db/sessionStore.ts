...
export const sessionStore = {
  ...
  create(data: Partial<SessionRow> & { id: string }): SessionRow {
    ...
    getDb().prepare(`INSERT INTO sessions (id, character_id, title, model, provider_id, workspace, parent_id, active_group, input_tokens, output_tokens, created_at, updated_at) VALUES (@id, @character_id, @title, @model, @provider_id, @workspace, @parent_id, @active_group, @input_tokens, @output_tokens, @created_at, @updated_at)`).run(row)
    return row
  },
  update(id: string, patch: Partial<SessionRow>): SessionRow | null {
    ...
    getDb().prepare(`UPDATE sessions SET character_id=@character_id, title=@title, model=@model, provider_id=@provider_id, workspace=@workspace, parent_id=@parent_id, active_group=@active_group, input_tokens=@input_tokens, output_tokens=@output_tokens, updated_at=@updated_at WHERE id=@id`).run(updated)
    return updated
  },
